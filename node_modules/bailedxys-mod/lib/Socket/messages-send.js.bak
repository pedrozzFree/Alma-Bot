...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        // Certifique-se de que esses imports estão no topo do seu arquivo

bily: async (jid, content, extra = {}, options = {}) => {
const { 
    isJidGroup, 
    getBinaryNodeChild 
} = require('baileys-mod/lib/WABinary');

const { generateWAMessage, generateMessageIDV2 } = require('baileys-mod/lib/Utils');
    const userJid = authState.creds.me.id; // seu jid
    let mediaHandle;

    // Garante que efêmero é setado se for grupo
    if (!options.ephemeralExpiration && isJidGroup(jid)) {
        ...
        const expiration = metadata?.child?.find(c => c.tag === 'ephemeral')?.attrs?.expiration || 0;
    ...
    // Configurações avançadas que o sendMessage normal não faz
    const additionalAttributes = {};
    const additionalNodes = [];

    // Delete remoto mesmo sem ser você
    if (content.delete && !content.delete.fromMe) additionalAttributes.edit = '8';
    if (content.edit) additionalAttributes.edit = '1';
    if (content.pin) additionalAttributes.edit = '2';
    if (content.keep) additionalAttributes.edit = '6';
    ...
    // Node custom
    additionalNodes.push({ tag: 'custom', attrs: { secret: 'true' }, content: [{ tag: 'inner', attrs: {} }] });
