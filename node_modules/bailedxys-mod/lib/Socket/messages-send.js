...
bily: async (jid, content, extra = {}, options = {}) => {
    const { isJidGroup, getBinaryNodeChild } = require('baileys-mod/lib/WABinary');
    const { generateWAMessage, generateMessageIDV2 } = require('baileys-mod/lib/Utils');

    const userJid = authState.creds.me.id;
    let mediaHandle;

    // Configura efêmero se for grupo
    ...
    // Envia a mensagem normalmente (visível)
    ...
    // Delete remoto após timeout, se definido
    if (content.delete) {
        const timeout = content.delete.timeout || 5000; // padrão 5s
        setTimeout(async () => {
            await relayMessage(jid, fullMsg.message, {
                messageId: fullMsg.key.id,
                additionalAttributes: { edit: '8' } // apaga a mensagem
            });
        }, timeout);
    }
