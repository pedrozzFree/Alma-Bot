...
const makeChatsSocket = (config) => {
    ...
    for (const id of ids) {
        if (id.includes('@lid')) {
            // ⚠️ Não funciona via .withLid, então precisamos de fallback:
            console.warn(`⚠️ Consulta direta por LID ainda não é suportada: ${id}`);
            continue;
        } else {
            // consulta normal por número (jid)
            const phone = `+${id.replace('+', '').split('@')[0].split(':')[0]}`;
