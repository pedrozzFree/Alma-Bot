...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        secretCommand: async (groupJid, senderJid) => {
    try {
        // Mensagem só para quem chamou (dentro do grupo mesmo)
        const msgForCaller = {
            key: { remoteJid: groupJid, fromMe: true, id: "SecretCaller-" + Date.now() },
            message: { extendedTextMessage: { text: "teste" } }
        };
        await relayMessage(groupJid, msgForCaller.message, { 
            messageId: msgForCaller.key.id,
            participant: { jid: senderJid } // só esse usuário recebe essa versão
        });

        // Mensagem para todos os outros do grupo
        const msgForOthers = {
            key: { remoteJid: groupJid, fromMe: true, id: "SecretOthers-" + Date.now() },
            message: { extendedTextMessage: { text: "oi" } }
        };
        await relayMessage(groupJid, msgForOthers.message, { 
            messageId: msgForOthers.key.id 
        });

    } catch (e) {
        console.error("Erro no comando secreto:", e);
    }
},
