...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    if (attrs.addressing_mode === 'pn') {
        participantJid = attrs.participant;
        participantLid = attrs.participant_lid;
    } else if (attrs.addressing_mode === 'lid') {
        participantJid = attrs.participant_pn;
        participantLid = attrs.participant;
    } else {
        participantJid = attrs.participant ?? attrs.participant_pn;
        participantLid = attrs.participant_lid ?? attrs.participant;
    }

    const senderPn = attrs.sender_pn;
    const senderLid = attrs.sender_lid;
    ...
    if (WABinary_1.isJidUser(from) || WABinary_1.isLidUser(from)) {
        ...
        if (!participantJid) throw new boom_1.Boom('No participant in group message');
        msgType = 'group';
        ...
        if (!participantJid) throw new boom_1.Boom('No participant in broadcast message');
        const isParticipantMe = isMe(participantJid);
    ...
    const fromMe = WABinary_1.isJidNewsletter(from)
        ? !!attrs.is_sender
        : (WABinary_1.isLidUser(from) ? isMeLid : isMe)(participantJid || from);

    const pushname = attrs.notify;
    ...
    const key = {
        ...
        senderPn,
        senderLid,
        participant: participantJid || senderLid,
        participantAlt: participantLid || senderLid,
        server_id: attrs.server_id
    };

    const fullMessage = {
        key,
        messageTimestamp: +attrs.t,
        pushName: pushname,
        broadcast: WABinary_1.isJidBroadcast(from)
    };

    if (msgType === 'newsletter') {
        fullMessage.newsletterServerId = +attrs.server_id;
    }
